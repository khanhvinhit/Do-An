    ...
    public partial class FrmBaiHocChiTiet : XtraForm
    {
                            ...
                            if (itemdmc.BaiKhoas.Count >= 1)
                            {
                                foreach (BaiKhoa bk in BaiKhoaDao.Instance.LoadBaiKhoas(itemdmc.ID))
                                {
                                    if (bk.NoiDung == null)
                                    {
                                        if (bk.HoiViet != null)
                                        {
                                            rkqBaiHoc.Text += bk.TraLoiKHo != null ? rkqBaiHoc.Text += "\t- " + bk.HoiKHo + "\t\t+ " + bk.TraLoiKHo + "\n" : rkqBaiHoc.Text += "\t- " + bk.HoiKHo + "\n";
                                            rkqBaiHoc.Text += bk.TraLoiViet != null ? "\t- " + bk.HoiViet + "\t\t+ " + bk.TraLoiViet + "\n" : rkqBaiHoc.Text += "\t- " + bk.HoiViet + "\n";
                                        }
                                        else
                                        {
                                            rkqBaiHoc.Text += bk.TraLoiKHo != null ? "\t- " + bk.HoiKHo + "\t\t+ " + bk.TraLoiKHo + "\n" : rkqBaiHoc.Text += "\t- " + bk.HoiKHo + "\n";
                                        }
                                    }
                                    else
                                    {
        ...
        private void lbcMucLuc_Click(object sender, EventArgs e)
        {
            ...
            if (subStringItem == "Câu hỏi")
            {
                ...
                if (f != null)
                {
                    XtraMessageBox.Show("Bạn đã mở câu hỏi!", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                }
                else
                {
